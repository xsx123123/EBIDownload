# Polariseq 外部依赖跨平台分发问题描述

## 背景

Polariseq 是一个用 **Rust + Tauri v2** 构建的跨平台桌面应用（支持 Windows / macOS / Linux），用于从 EBI/NCBI 高速下载生物测序数据。
...
- 当前必须手动：装 Conda → 创建环境 → `conda install -c bioconda sra-tools` → 再手动写 `polariseq.yaml` 配置绝对路径（`.fastq` → `.fastq.gz` 的压缩已由 Rust 原生并行 gzip 替代，无需再安装 `pigz`）
...
### 关键需求
1. **自动分发**：应用首次启动时，自动将所需工具下载/解压到应用私有目录（如 `~/Library/Application Support/Polariseq/bin/` 或 `%APPDATA%\Polariseq\bin\`）
...
2. **`pigz` 的跨平台替代方案** ✅ 已解决
   - 已引入 `gzp` crate（默认 `deflate_default` + `libdeflate` 后端）作为 Rust 原生并行 gzip 实现，替代外部 `pigz`。
   - 新增 `polariseq_core::compress_fastq_files()` 统一处理 `.fastq` → `.fastq.gz` 的压缩，并在 async 上下文中通过 `tokio::task::spawn_blocking` 执行。
   - 已从依赖检查和文档中移除 `pigz`。
...
核心诉求：**如何让一个 Tauri 桌面应用在不依赖用户手动安装 Conda/系统包的前提下，在 Windows 和 macOS 上自动获得 `prefetch`、`fasterq-dump` 的运行能力。** 需要兼顾法律许可、技术可行性、用户体验和平台安全限制。（`pigz` 已由 Rust 原生并行 gzip 替代，不再是外部依赖。）
