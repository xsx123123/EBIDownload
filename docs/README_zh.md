...
- **断点续传**: 在 `ascp` 和 `prefetch` 下载模式下支持断点续传，保障下载的连续性。
...
**关于数据完整性的说明**: 为保证下载数据的完整性，建议在下载完成后进行MD5校验。

---
## 5. 输出文件结构

脚本运行后, 输出目录中将包含以下文件和目录:
...
- **MD5校验文件**: `R1_fastq_md5.tsv` 和 `R2_fastq_md5.tsv`
  - 这两个文件包含了从 EBI 数据库中获取的官方 MD5 校验码和样本名称，分别对应 R1 和 R2 的 FASTQ 文件。你可以使用这些文件来校验下载数据的完整性。

- **样本目录**: `SRRXXXXXX/`
  - 每个目录对应一个下载的样本 (Run ID)，其中包含了实际的测序数据文件。